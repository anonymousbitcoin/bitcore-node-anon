...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getMNList', this, this.getMNList, 0],
    ['getListMasternodes', this, this.getListMasternodes, 0],
...
Bitcoin.prototype._normalizeAddressArg = function(addressArg) {
  ...
  return addresses;
  // // Reserialize addresses from BTC or ZCL to anon
  // const anonVersionBytes = Buffer.from([0x13, 0x25]);
  // let anonAddresses = [];
  ...
  //       let newBuffer = Buffer.concat([anonVersionBytes, raw.slice(1)]);
  //       anonAddresses.push(bitcore.encoding.Base58Check.encode(newBuffer).toString());
  //     } else {
  //       let firstTwo = raw.slice(0,2).toString('hex')
  //       if (firstTwo === '1325' || firstTwo === '13af') { // anon
  //         anonAddresses.push(a);
  //       } else if (firstTwo === '1cb8' || firstTwo === '1cbd') { // ZCL
  //         let newBuffer = Buffer.concat([anonVersionBytes, raw.slice(2)]);
  //         anonAddresses.push(bitcore.encoding.Base58Check.encode(newBuffer).toString());
  //       } else {
...
Bitcoin.prototype.getListMasternodes = function(callback) {
  var self = this;
  this.client.listmasternodes(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
