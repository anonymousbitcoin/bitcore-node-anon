...
Bitcoin.prototype.startAll = function(callback){
	var self = this;
	this.client.masternode("start-all", function(err, response){
		if (err) {
			return callback(self._wrapRPCError(err));
		}
		callback(null,response.result)
...
Bitcoin.prototype.listUnspent = function(callback){
	var self = this;
	this.client.listUnspent(function(err, response){
		if(err){
			return callback(self._wrapRPCError(err));
		}
		callback(null, response.result);
