...
var BitcoinRPC = require('bitcoind-rpc-anon');
var $ = bitcore.util.preconditions;
var _  = bitcore.deps._;
var Transaction = bitcore.Transaction;
console.log("free fall flow", bitcore.GovObject);
...
Bitcoin.prototype._initCaches = function() {
  ...
  this.sporksListCache = LRU(50000);
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['govObjectList', this, this.govObjectList, 1],
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      console.log("INSIDE OF FIRST ASYNC FUNNC");
      ...
      console.log("AFTER NNODE.CLIENT IS CREATED");
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  console.log("OUTSIDE OF ASYNC FUNC");
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    console.log("EFORE ITCOIN RPC PROCESS STARTED");
    node.client = new BitcoinRPC({
    ...
    console.log("ITCOIN RPC PROCESS STARTED");
    self._loadTipFromNode(node, done);
...
Bitcoin.prototype._normalizeAddressArg = function(addressArg) {
  ...
  //     let raw = bitcore.encoding.Base58Check.decode(a);
  
  //     let first = raw.slice(0,1).toString('hex');
  //     if (first === '00' || first === '05') { // BTC
  //       let newBuffer = Buffer.concat([btcpVersionBytes, raw.slice(1)]);
  //       btcpAddresses.push(bitcore.encoding.Base58Check.encode(newBuffer).toString());
  //     } else {
...
Bitcoin.prototype.getMNList = function(callback){
  ...
  var getStatus = function(next){
      ...
      response ? console.log("response is =====>", response) : console.log("no available  response ");
		  if(response && response.result){
        console.log("response this time", response);
        console.log("result this time", response.result);
			  var result = response.result;
  ...
  var prepareResponse = function(err){
    ...
    console.log("keys.indexOf('rank') > -1 = ", keys.indexOf('rank') > -1 &&
    ...
    if(
      ...
      console.log("WArgadfgha", rawResults.lastseen);
        var lastseenKeys = Object.keys(rawResults.lastseen);
        var lastseenLength = lastseenKeys.length;
        console.log(lastseenLength);
        
        //We get threw all vins by lastseen 
        for(var i = 0; i<lastseenLength; i++){
          var vin = lastseenKeys[i];
          ...
          var MN = {
            ...
            console.log("I MAKE IT HERE 2");
            console.log("MNNNNNN", MN);
          MNList.push(MN);
        }
        console.log(MNList);
    }else{
  ...
  console.log("MNListData ============>", MNListData);
  if (MNListData) {
      return setImmediate(function() {
